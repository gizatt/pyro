...
class ConditionalFlowTests(TestCase):
    def setUp(self):
        # Epsilon is used to compare numerical gradient to analytical one
        self.epsilon = 1e-4
    ...
    def _test_jacobian(self, input_dim, observed_dim, make_flow):
        jacobian = torch.zeros(input_dim, input_dim)
    ...
    def _test_inverse(self, input_dim, observed_dim, make_flow):
        base_dist = dist.Normal(torch.zeros(input_dim), torch.ones(input_dim))
        flow = make_flow(input_dim, observed_dim)
        if observed_dim > 0:
    ...
    def _test_shape(self, base_shape, observed_dim, make_flow):
        base_dist = dist.Normal(torch.zeros(base_shape), torch.ones(base_shape))
        last_dim = base_shape[-1] if isinstance(base_shape, tuple) else base_shape
        flow = make_flow(input_dim=last_dim, observed_dim=observed_dim)
        if observed_dim > 0:
            flow.set_z(torch.randn(base_shape[:-1] + (observed_dim,)))
        sample = dist.TransformedDistribution(base_dist, [flow]).sample()
        assert sample.shape == base_shape

    def _make_maf(self, input_dim, observed_dim):
        arn = AutoRegressiveNN(input_dim, [3 * input_dim + 1],
    ...
    def test_maf_jacobians(self):
        for input_dim in [2, 3, 5, 7, 9, 11]:
            for observed_dim in [0, 3]:
                self._test_jacobian(input_dim, observed_dim, self._make_maf)

    def test_maf_inverses(self):
        for input_dim in [2, 3, 5, 7, 9, 11]:
            for observed_dim in [0, 3]:
                self._test_inverse(input_dim, observed_dim, self._make_maf)

    def test_maf_shapes(self):
        for shape in [(3,), (3, 4), (3, 4, 2)]:
            for observed_dim in [0, 5]:
                self._test_shape(shape, observed_dim, self._make_maf)
